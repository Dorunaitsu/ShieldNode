...
- Resolve an unknown `shieldnode_...` key on its own, in one call, instead of asking you which service it belongs to.
...
- Handle the push-approval flow end to end: detect `403 approval_required`, send `X-Agent-Name` and `X-Approval-Duration`, poll without hammering, surface a clean message when you decline or it times out.
- Ask for a recurring access window when the job is a cron, instead of firing a push at 3am every night.
- Propose a service you have not configured yet, so you approve a prefilled screen on your phone and type only your own credential. The agent never sees it.
...
Then point your agent at it manually (system prompt, custom instructions). `SKILL.md` is self-contained for everyday use, but the links into `references/` will not resolve, so prefer the full install above if your agent does deep debugging.
...
| File             | Purpose                                                           |
|------------------|-------------------------------------------------------------------|
| `SKILL.md`       | The operational skill, read by the agent. Kept small on purpose so it costs little to keep loaded |
| `references/`    | Depth loaded only when a task needs it: approval recipe, troubleshooting, per-service docs, manual dashboard setup |
...
- Get the app: [shieldnode.app/get-app](https://shieldnode.app/get-app)
